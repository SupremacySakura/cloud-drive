...
func (r *FileRepository) GetChildrenByFolderIDAndUserID(folderID uint, userID uint) ([]model.FolderModel, []model.FileModel, error) {
	var folders []model.FolderModel
	if err := r.DB.Where("parent_id = ? AND user_id = ?", folderID, userID).Find(&folders).Error; err != nil {
		return nil, nil, err
	}

	var files []model.FileModel
	if err := r.DB.Where("folder_id = ? AND user_id = ?", folderID, userID).Find(&files).Error; err != nil {
		return nil, nil, err
	}
