...
const getIconForContentType = (filename: string, contentType: string) => {
  ...
  else if (contentType.includes('pdf') || contentType.includes('word') || contentType.includes('excel') || contentType.includes('powerpoint') || contentType.startsWith('text/')) fileType = 'document'
...
const updateDigit = (index: number, value: string) => {
  const next = value.replace(/[^a-zA-Z0-9]/g, '').slice(-1).toUpperCase()
...
const handlePaste = (event: ClipboardEvent) => {
  ...
  const chars = text.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6).split('')
  ...
  <div
    ...
    <main class="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 relative z-10">
      ...
      <div class="w-full max-w-[480px] space-y-8">
        <!-- Entry Card -->
        <div class="bg-white dark:bg-slate-900/50 p-8 rounded-xl border border-primary/10 shadow-xl shadow-primary/5">
          ...
          <div class="space-y-6">
            <div class="flex justify-between gap-2 sm:gap-4">
              <input v-for="(_, index) in codeDigits" :key="index"
                ...
                maxlength="1" inputmode="text" placeholder="·" type="text" :value="codeDigits[index]"
            ...
            <button
              ...
              :disabled="!canSubmit" @click="handleRetrieve">
        ...
        <div v-if="successState" class="space-y-4">
          ...
          <div
            ...
            <div
              ...
              <div class="absolute bottom-1 right-1 bg-current text-[8px] text-white px-1 rounded font-bold"
            ...
            <div class="flex-1 text-center sm:text-left overflow-hidden">
              <h4 class="text-slate-900 dark:text-slate-100 font-bold truncate text-lg" :title="downloadedFile.name">
                {{ downloadedFile.name }}
              </h4>
              <p class="text-slate-500 dark:text-slate-400 text-sm">{{ downloadedFile.size }} • 下载成功</p>
            </div>
            <button
    ...
    <div class="fixed inset-0 pointer-events-none z-[-1] overflow-hidden">
      ...
      <div class="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-primary/5 rounded-full blur-[120px]"></div>
    </div>
  </div>
</template>
