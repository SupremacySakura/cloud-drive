...
type FileFilterKey = 'all' | 'folder' | 'image' | 'video' | 'audio' | 'document' | 'other'
...
const fileFilterOptions: Array<{ key: FileFilterKey; label: string }> = [
  { key: 'all', label: '全部类型' },
...
const matchesActiveFilter = (item: DisplayItem) => {
  if (activeFilter.value === 'all') return true
  if (activeFilter.value === 'folder') return item.type === 'folder'
  return item.type === 'file' && item.file_type === activeFilter.value
}
...
const sortedFiles = computed(() => {
  ...
  const searched = query ? mapped.filter(item => item.name.toLowerCase().includes(query)) : mapped
...
const activeFilterLabel = computed(() => {
  return fileFilterOptions.find(option => option.key === activeFilter.value)?.label || '全部类型'
})
...
const processUploadTask = async (task: UploadTask): Promise<void> => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      uploadFile(task.file, config, progress => {
    ...
    <template v-else>
      <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          <div
            class="mb-6 flex flex-col gap-4 lg:mb-8 lg:flex-row lg:items-center lg:justify-between"
          >
            <div>
          ...
          >
            <div class="flex w-full flex-wrap items-center gap-2 lg:w-auto">
              ...
              <div class="relative" @click="onStopPropagation">
                ...
                >
                  <button
                    v-for="option in fileFilterOptions"
                    :key="option.key"
            ...
            <div class="relative w-full sm:w-auto" @click="onStopPropagation">
              <div
                class="flex items-center gap-2 text-xs font-medium text-slate-400 sm:justify-end"
              >
                <span>Sorted by</span>
                <button
                  ...
                  >
                    <td class="py-4 px-6">
                      <input
                        ...
                        @click.stop
                        @change="
                          toggleOne(String(file.id), ($event.target as HTMLInputElement).checked)
                        "
                      />
                    </td>
                    <td class="py-4 px-4">
          ...
          <div v-else>
              ...
              >
                ...
                <div class="mt-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    type="button"
                    @click.stop="e => openFileMenu(file, e)"
                  >
            ...
            <div
              ...
              class="mt-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4 dark:border-slate-800 dark:bg-slate-950 sm:flex-row sm:items-center sm:justify-between sm:px-6"
            >
              <p class="text-xs text-slate-500">
                Showing
                <span class="font-bold text-slate-900 dark:text-slate-100"
        ...
        >
          <div
            class="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xl w-full max-w-2xl p-6"
            @click.stop
          >
            ...
            <div class="mb-3 flex items-center justify-between gap-3">
              <nav class="flex items-center gap-2 text-xs text-slate-500 overflow-x-auto">
                <template v-for="(bc, idx) in moveBrowserBreadcrumbs" :key="`${bc.id}-${idx}`">
                  <button
                    v-if="idx < moveBrowserBreadcrumbs.length - 1"
            ...
            <div class="rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div class="max-h-72 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <div
                  v-if="!isMoveBrowserLoading && moveBrowserFolders.length === 0"
        ...
        >
          <div
            class="bg-white dark:bg-slate-950 w-full max-w-6xl h-full max-h-[850px] rounded-xl shadow-2xl flex flex-col overflow-hidden relative border border-slate-200 dark:border-slate-800"
            @click.stop
          >
            <div
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-slate-400">
                    {{ previewingFile?.typeLabel || 'File' }} •
                    {{ formatBytes(previewingFile?.size || 0) }}
                  </p>
                </div>
              </div>
              <button
            ...
            <div class="flex flex-1 overflow-hidden">
              <div
                class="flex-1 bg-slate-50 dark:bg-slate-900/30 relative flex flex-col overflow-hidden"
              >
                <div class="flex-1 overflow-y-auto p-6 md:p-8 pb-24">
                  ...
                  >
                    <video
                      controls
                      :src="previewUrl"
                      class="max-w-full rounded-lg bg-black"
                    ></video>
                  </div>
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="font-semibold text-slate-900 dark:text-slate-100">
                上传任务 ({{ uploadTasks.filter(t => t.status === 'success').length }}/{{
                  uploadTasks.length
                }})
              </span>
            </div>
                ...
                <div class="flex items-center justify-between mb-1">
                  ...
                  <span
                    ...
                    :class="{
                      ...
                      'text-primary': ['pending', 'hashing', 'uploading', 'merging'].includes(
                        task.status,
                      ),
                    }"
                  >
